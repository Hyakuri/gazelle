...
The integration keeps the model boundary deliberately small: fusion/tracking produces an ordered `HeadObservation(person_id, bbox, confidence)` tuple for `GazellePredictor.predict_frame(...)` and an aligned `HeadPerception` tuple for observation sidecars and rendering. Only the normalized head bbox, person ID, and optional confidence cross the predictor boundary; face bbox, face mesh, face keypoints, pose landmarks, head pose, and tracking state are not added to the Gazelle tensor input. Gazelle returns its heatmap, gaze peak, peak value, and optional in/out score in the same person order. This lets face and pose evidence improve head localization while preserving the original Gazelle model contract.
...
When a `HeadPerception` result is supplied, its primary head bbox is drawn by default, independently of the opt-in Gazelle `--head-box`. The perception controls are `--face-box` (auxiliary face bbox, default off), `--face-keypoints` (up to six detector keypoints, default off), `--pose-head-points` (pose head and shoulder points, default off), `--face-mesh` (current in-memory face landmarks, default off), and `--no-track-state` (disable the person ID, perception state, and confidence label, which is on by default). A `tracked_only` primary box is dashed and uses reduced alpha to distinguish bridged tracking evidence from a current observation.
...
Perception and tracking run exactly once on every decoded and written frame. `--frame-step` gates only Gazelle: skipped frames still get their normal `head_observations.jsonl` row, then a `predictions.jsonl` row with `status="skipped"`, even when no head was observed. Only frames with gaze `status="ok"` pass their perceptions to the renderer. Skipped and `no_head` frames are copied unchanged into the rendered video. `--max-frames` limits both JSONL files and the optional rendered video to the same written-frame count. `--output-fps` is used only when the source video FPS is invalid; otherwise the source FPS is preserved. `--save-heatmaps` is not supported for video in this milestone and exits with `video heatmap export is not implemented yet`.
